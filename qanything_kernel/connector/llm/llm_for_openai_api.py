...
class OpenAILLM(BaseAnswer, ABC):
    ...
    def num_tokens_from_messages(self, messages, model=None):
        ...
        if model in {
            ...
            tokens_per_message = 4  # 每条消息遵循{ROLE/NAME}\N{CONTENT}\N格式

            tokens_per_name = -1  # 如果有名字，角色会被省略

        elif "gpt-3.5-turbo" in model: # 对于GPT-3.5-TURBO模型可能会有更新，此处返回假设为GPT-3.5-TURBO-0613的TOKEN数量，并给出警告

            logging.info("Warning: gpt-3.5-turbo may update over time. Returning num tokens assuming gpt-3.5-turbo-0613.")

            return self.num_tokens_from_messages(messages, model="gpt-3.5-turbo-0613")

        elif "gpt-4" in model: # 对于GPT-4模型可能会有更新，此处返回假设为GPT-4-0613的TOKEN数量，并给出警告
        ...
        for message in messages:
            ...
            if isinstance(message, dict):
                ...
                NotImplementedError(f"""num_tokens_from_messages() is not implemented message type {type(message)}. """)
