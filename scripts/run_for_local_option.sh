...
update_or_append_to_env() {
  ...
  # 检查键是否存在于.ENV文件中
  if grep -q "^${key}=" "$env_file"; then # 如果键存在，则更新它的值
    sed -i "/^${key}=/c\\${key}=${value}" "$env_file"
  else # 如果键不存在，则追加键值对到文件
    echo "${key}=${value}" >> "$env_file"
  fi

}

check_log_errors() { # 参数为日志文件名
...
# 计算FASTCHAT文件夹下所有文件的MD5校验和(先计算所有单个文件的MD5然后再计算MD5的MD5值)
...
# 检查两个校验和是否相等，如果不相等则表示`third_party/FastChat/fastchat/conversation.py`注册了新的CONV_TEMPLATE，则需重新安装依赖
...
gpu_series=$(echo $gpu_model | grep -oP '(RTX\s*(30|40)|A(10|30|40|100|800))') # RTX 40
...
# 如果`compute_capability`为空，则说明显卡型号不在`gpu_capabilities.json`中
...
# gpu1 model: NVIDIA GeForce RTX 4090
...
# OCR_USE_GPU=True because 8.9 >= 7.5

update_or_append_to_env "OCR_USE_GPU" "$OCR_USE_GPU"

####################################################################
# 使用NVIDIA-SMI命令获取GPU的显存大小（以MiB为单位）：24564
...
if [ "$runtime_backend" = "default" ] && [ "$model_size_num" -ne 0 ]; then
    if [ -z "$gpu_series" ]; then  # 不是NVIDIA30系列或40系列
        echo "默认后端为`FasterTransformer`，仅支持NVIDIA RTX 30系列或40系列显卡，您的显卡型号为： $gpu_model, 不在支持列表中，将自动为您切换后端："
        ...
        if [ "$GPU1_MEMORY_SIZE" -ge 24000 ] && [ $(echo "$compute_capability >= 8.6" | bc) -eq 1 ]; then
            ...
            # 自动切换HUGGINGFACE后端
            echo "根据匹配算法，已自动为您切换为HUGGINGFAC后端"
            runtime_backend="hf"
        fi
    fi
fi
...
while true; do
    ...
    if [ "$runtime_backend" = "default" ]; then
        if [ $gpu_id1 -eq $gpu_id2 ]; then # 单GPU
    ...
    if [ $elapsed_time -ge 120 ]; then
        ...
        echo "启动EMBEDDING和RERANK服务超时，自动检查 $embed_rerank_log_file 中是否存在ERROR..."
    ...
    if [ $embed_rerank_response -eq 200 ]; then
        ...
        echo "EMBEDDING和RERANK服务已准备就绪！(7.5/8)"
    ...
    echo "EMBEDDING和RERANK服务正在启动，可能需要一段时间...你有时间去冲杯咖啡 :)"
...
while true; do
    ...
    if [ $elapsed_time -ge 300 ]; then
        ...
        echo "启动LLM服务超时，自动检查 $llm_log_file 中是否存在ERROR..."
    ...
    if [ "$runtime_backend" = "default" ]; then
        if [ $llm_response -eq 200 ]; then
            ...
            echo "LLM服务已准备就绪！现在您可以使用QANYTHING服务。（8/8)"
            break
        fi
    else
    ...
    echo "LLM服务正在启动，可能需要一段时间...你有时间去冲杯咖啡 :)"
    sleep 10
done
